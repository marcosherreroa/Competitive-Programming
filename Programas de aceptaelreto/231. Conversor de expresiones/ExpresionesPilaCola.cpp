...
bintree<char> toTree(std::stack<char> & pila) {
	char raiz = pila.top();
	pila.pop();

	if (isdigit(raiz)) return { {}, raiz,{} };
	else {
		bintree <char> right = toTree(pila);
		bintree<char> left = toTree(pila);
		return { left, raiz, right };
	}
}

void recorridoPorNiveles(bintree<char> const & tree, std::stack<char> &sol, std::queue<bintree<char>> & cola) {
	if (!tree.empty()) {
		sol.push(tree.root());
		cola.push(tree.left());
		cola.push(tree.right());

		while (!cola.empty()) {
			bintree<char> aux = cola.front();
			cola.pop();
			recorridoPorNiveles(aux, sol, cola);
		}
	}
}
...
bool resuelveCaso() {
	...
	bintree<char> tree = toTree(pila);
	std::stack<char> sol; std::queue<bintree<char>> cola;
	 recorridoPorNiveles(tree, sol, cola);
